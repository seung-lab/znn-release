...
private:
    std::mutex                 m_          ;
    std::map<vec3i, fftwf_plan> fwd_        ;
    std::map<vec3i, fftwf_plan> bwd_        ;
    real                     time_       ;

    static_assert(std::is_pointer<fftwf_plan>::value,
                  "fftw_plan must be a pointer");

public:
    ~fftw_plans_impl()
    {
        for ( auto& p: fwd_ ) fftwf_destroy_plan(p.second);
        for ( auto& p: bwd_ ) fftwf_destroy_plan(p.second);
        fftwf_cleanup();
    }
    ...
    fftwf_plan get_forward( const vec3i& s )
    {
        guard g(m_);

        fftwf_plan& ret = fwd_[s];
...
//                  << time_ << std::endl;
    ...
    fftwf_plan get_backward( const vec3i& s )
    {
        guard g(m_);

        fftwf_plan& ret = bwd_[s];
