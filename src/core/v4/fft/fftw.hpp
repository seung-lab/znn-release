...
#           endif
        ...
        cube_p<complex> forward( cube_p<double>&& in )
        {
        ...
        cube_p<complex> forward_pad( const ccube_p<double>& in )
        {
            cube_p<double> pin = pad_zeros(*in, sz);
            return forward(std::move(pin));
        }

        cube_p<double> backward( cube_p<complex>&& in )
        {
            cube_p<double> ret = get_cube<double>(sz);
            backward( *in, *ret );
            return ret;
        }
    };
...
public:
    static void forward( cube<double>& in,
                         cube<complex>& out )
    {
...
#       endif
    ...
    static cube_p<complex> forward( cube_p<double>&& in )
    {
        cube_p<complex> ret = get_cube<complex>(fft_complex_size(*in));
        fftw::forward( *in, *ret );
        return ret;
    ...
    static cube_p<double> backward( cube_p<complex>&& in, const vec3i& s )
    {
        cube_p<double> ret = get_cube<double>(s);
        fftw::backward( *in, *ret );
        return ret;
    }

    static cube_p<complex> forward_pad( const ccube_p<double>& in,
                                        const vec3i& pad )
    {
        cube_p<double> pin = pad_zeros(*in, pad);
