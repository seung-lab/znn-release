...
public:
    void dispatch(const ccube_p<double>& v) const
    {
...
private:
    void fft_dispatch( ccube_p<double> const & v,
                       vec3i const & s,
...
public:
    void dispatch( ccube_p<double> const & v,
                   task_manager & manager) const
    {
        for ( auto& t: targets_ )
...
private:
    void fft_dispatch( const ccube_p<double>& v, const vec3i& s,
...
public:
    ...
    void dispatch(size_t i, const ccube_p<double>& v) const
    {
    ...
    template<class M>
    void dispatch(size_t i, ccube_p<double> const & v, M & man)
    {
        ZI_ASSERT(i<dispatchers_.size());
