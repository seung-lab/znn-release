...
public:
	void save( const std::string& fpath )
	{
		...
		vec3i sz = volume_utils::volume_size(inputs_.front());
		export_size_info(sz, fpath + ".input");

		// labels
		volume_utils::save_list(ret_lbls_, fpath + ".label");
		sz = volume_utils::volume_size(labels_.front());
...
private:
	...
	void update_return_labels()
	{
		if( affin_ )
		{
			...
			FOR_EACH( it, labels_ )
			{
				vec3i s = volume_utils::volume_size(*it);
			...
			FOR_EACH( it, masks_ )
			{
				vec3i s = volume_utils::volume_size(*it) - vec3i::one;				
